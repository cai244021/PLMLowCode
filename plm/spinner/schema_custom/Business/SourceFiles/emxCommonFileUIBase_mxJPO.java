        ...
        try
        {
            ...
            while( fileItr.hasNext() && formatItr.hasNext() && fileSizeItr.hasNext() )
            {
                ...
                if ( file != null && !"".equals(file) )
                {
                    ...
                    {
                       fileMap.put(OBJECT_MAP_ID, objectId+"~"+file+"~"+format);
                    }
                    else
                    {
                       fileMap.put(OBJECT_MAP_ID, objectId);
                    }
    ...
    public Vector getLockIcon(Context context, String[] args) throws Exception
    {
        ...
        try
        {
            ...
            for(int i=0; i< objectList.size(); i++)
            {
                ...
                if ("TRUE".equalsIgnoreCase(fileLocked)){
                    String titleText =  EnoviaResourceBundle.getProperty(context, EMX_COMPONENTS_STRING_RESOURCE, context.getLocale(), "emxComponents.Common.LockedBy");
                    statusImageString = "<img border=\"0\" src=\"../common/images/iconStatusLocked.gif\" alt=\"" + XSSUtil.encodeForHTMLAttribute(context, fileLocker) + "\" title=\"" +titleText+" : "+ XSSUtil.encodeForHTMLAttribute(context, fileLocker) + "\"/>";
                }
                showLock.add(statusImageString);
            }
            return  showLock;
  ...
  public Vector getFileName(Context context, String[] args)throws Exception
  {
      ...
      for(int i=0; i<ml.size(); i++)
      {
        ...
        //String sLink = strLink + "&parentOID="+objectMap.get("masterId")+"&objectId="+objectMap.get(DomainConstants.SELECT_ID)+"&AppendParameters=true&trackUsagePartId="+strPartId+"', '700', '600', 'false', 'popup', '')\">";
            ...
            strFileName=replaceSpecialCharsWithSpace(strFileName);
    ...
    public  String replaceSpecialCharsWithSpace(String str) {
        if (str == null) {
            return null;
        }
  ...
  public Vector getFileRevision(Context context, String[] args)throws Exception
    ...
    sl.add(DomainConstants.SELECT_ID);
      ...
      for(int i=0; i<objectList.size(); i++)
      {
        ...
        {
          String sLink = strLink + "&amp;parentOID="+objectMap.get("masterId")+"&amp;objectId="+objectMap.get(OBJECT_MAP_ID)+"', '700', '600', 'false', 'popup', '')\">"+(String)objectMap.get(DomainConstants.SELECT_REVISION)+"</a>";
...
//                        fileActionsStrBuff.append(viewerURL);
                        String viewerURL = getPdfViewUrl(context,masterId);
                        if (UIUtil.isNotNullAndNotEmpty(viewerURL)){
                            fileActionsStrBuff.append("<a href=\"#\" onclick=\"window.open('"+viewerURL+"', '_blank', 'width=800,height=600'); return false;\">");
                            fileActionsStrBuff.append("<img border='0' src='../common/images/iconActionView.gif' alt=\""
                                    + i18nViewerTip + "\" title=\"" + i18nViewerTip + "\"></img></a>&#160;");
                        }else {
//                            fileActionsStrBuff.append("<a href=\"#\" onclick=\"alert('没有包含 PDF 文档，不能预览');\">");
                            fileActionsStrBuff.append("<a href=\"#\" onclick=\"getTopWindow().showTransientMessage('没有包含 PDF 文档，不能预览', 'warning', 'alert-right-search'); return false;\">");
                            fileActionsStrBuff.append("<img border='0' src='../common/images/iconActionView.gif' alt=\""
                                    + i18nViewerTip + "\" title=\"" + i18nViewerTip + "\"></img></a>&#160;");
                        }
                    } else {
    ...
    */
        ...
        throws Exception
    {
        ...
        // get the Master Object meta data
        String canCheckin  = (String) masterMap.get(CommonDocument.SELECT_HAS_CHECKIN_ACCESS);
        String canLock     = (String) masterMap.get(CommonDocument.SELECT_HAS_LOCK_ACCESS);
        String canUnLock     = (String) masterMap.get(CommonDocument.SELECT_HAS_UNLOCK_ACCESS);
        String documentOwner = (String) masterMap.get(CommonDocument.SELECT_OWNER);
        String suspendVersioning     = (String) masterMap.get(CommonDocument.SELECT_SUSPEND_VERSIONING);
        boolean moveFilesToVersion = (Boolean.valueOf((String) masterMap.get(CommonDocument.SELECT_MOVE_FILES_TO_VERSION))).booleanValue();
        String fileId    = (String) masterMap.get(CommonDocument.SELECT_ID);
         String isLast = "false";
        ...
        while(revisionsListItr.hasNext())
        {
            ...
            isLast = (String)revisionMap.get("islast");
            ...
            String versionFileRevision = (String)revisionMap.get(CommonDocument.SELECT_REVISION);
            String verTitle = (String) revisionMap.get(CommonDocument.SELECT_TITLE);
            String strOwner = (String) revisionMap.get(CommonDocument.SELECT_OWNER);
            ...
            revisionMap.put("DocumentOwner", documentOwner); 
            revisionMap.put("masterId", masterId);
            revisionMap.put("fileId", fileId);
            revisionMap.put("islast",isLast);
            revisionMap.put(CommonDocument.SELECT_FILE_FORMAT, fileFormat);
            revisionMap.put(CommonDocument.SELECT_FILE_NAME, versionFileName);
            revisionMap.put(CommonDocument.SELECT_REVISION, versionFileRevision);
            //revisionMap.put(CommonDocument.SELECT_FILE_SIZE, fileSize);
            revisionMap.put(CommonDocument.SELECT_SUSPEND_VERSIONING, suspendVersioning);
            revisionMap.put(CommonDocument.SELECT_HAS_CHECKIN_ACCESS, canCheckin);
            revisionMap.put(CommonDocument.SELECT_HAS_LOCK_ACCESS, canLock);
            revisionMap.put(CommonDocument.SELECT_HAS_UNLOCK_ACCESS, canUnLock);
            revisionMap.put(CommonDocument.SELECT_OWNER,strOwner);
                            ...
                            throws Exception
    {
      ...
      }catch(Exception e)
      {
            ...
            while(objectListItr.hasNext())
            {
                ...
                //fileFormat = (String) objectMap.get(CommonDocument.SELECT_FILE_FORMAT);
                String isLastVersion = (String)objectMap.get("islast");
                if("true".equalsIgnoreCase(isLastVersion)){
                    fileFormat = (String) objectMap.get(CommonDocument.SELECT_FILE_FORMAT);
                    encodedFormat = fileFormat;
                }
                else{
                    DomainObject domainObject=DomainObject.newInstance(context, versionId);
                    fileFormat = CommonDocument.getFileFormat(context,domainObject);
                    encodedFormat = fileFormat;
                }
                ...
                if ( CommonDocument.canView(context, objectMap)&& docHasFiles )
                {
                ...
                if ( canViewAndDownload && !UINavigatorUtil.isMobile(context) && docHasFiles)
                {
                    ...
                    if ( !isprinterFriendly )
                    {
                      downloadURL = "javascript:callCheckout('"+ masterId +"','download', '"+ XSSUtil.encodeForJavaScript(context, fileName)+ "', '" + fileFormat +"', null, null, null, null, '"+strPartId+"');";
                ...
                if (isLatestVersion && CommonDocument.canCheckout(context, objectMap))
                {
                  if ( !isprinterFriendly )
                  {
                ...
                else if (isLatestVersion && CommonDocument.canCheckin(context, objectMap))
                {
                ...
                fileActions = fileActionsStrBuff.toString();
                System.out.println("fileActions : "+fileActions);
                fileActionsVector.add(fileActions);
            }
        }
        catch(Exception ex)
        ...
        try
        {
            ...
            while(versionItr.hasNext())
            {
                ...
                for (int j=0; j<revisionsList.size(); j++)
                {
                    ...
                    isLast = (String)fileVersionMap.get("islast");
                    ...
                    fileVersionMap.put("islast",isLast);
                    fileVersionMap.put(CommonDocument.SELECT_FILE_FORMAT, fileFormat);
                    fileVersionMap.put(CommonDocument.SELECT_FILE_NAME, versionFileName);
                    fileVersionMap.put(CommonDocument.SELECT_REVISION, versionFileRevision);
                    //fileVersionMap.put(CommonDocument.SELECT_FILE_SIZE, fileSize);
                    ...
                    fileVersionMap.put(CommonDocument.SELECT_HAS_LOCK_ACCESS, canLock);
                    fileVersionMap.put(CommonDocument.SELECT_HAS_UNLOCK_ACCESS, canUnLock);
                    fileVersionMap.put(CommonDocument.SELECT_OWNER,(String)fileActiveVersionMap.get(CommonDocument.SELECT_OWNER));
                    fileMapList.add(fileVersionMap);
                }
  ...
  /**
            ...
            if ( formatDetailsMap == null )
            {
                ...
                while (itr.hasNext())
                {
                    ...
                    if ( viewer == null || "".equals(viewer) || "null".equals(viewer) || viewerURL.equals(viewer) )
                    {
                        ...
                        {
                            if(!bUIType)
                                viewerURL = "<a href=\"javascript:getTopWindow().callCheckout('"+XSSUtil.encodeForJavaScript(context, objectId)+"','view', '"+XSSUtil.encodeForJavaScript(context, fileName)+"', '"+XSSUtil.encodeForJavaScript(context, format)+"', null, null, null, null, '"+XSSUtil.encodeForJavaScript(context, partId)+"');\"><img src=\"../common/images/iconActionView.gif\" border=\"0\" alt=\""+i18nViewerTip+"\" title=\""+i18nViewerTip+"\"></img></a>";
                            else
                                viewerURL = "<a href=\"javascript:openViewer('"+ XSSUtil.encodeForJavaScript(context, objectId) +"','view', '"+ XSSUtil.encodeForJavaScript(context, fileName)+ "', '" + XSSUtil.encodeForJavaScript(context, format) +"', null, null, null, null, '"+XSSUtil.encodeForJavaScript(context, partId)+"');\"><img src=\"../common/images/iconActionView.gif\" border=\"0\" alt=\""+i18nViewerTip+"\" title=\""+i18nViewerTip+"\"></img></a>";
                        } else {
                            viewerURL = servletPreFix + viewerURL + "?action=view&amp;";
                            viewerURL = "<a href=\"javascript:callViewer('"+ XSSUtil.encodeForJavaScript(context, objectId) +"','view', '"+ XSSUtil.encodeForJavaScript(context, fileName)+ "', '" + XSSUtil.encodeForJavaScript(context, format) +"', '"+ XSSUtil.encodeForJavaScript(context, viewerURL) +"', '"+ XSSUtil.encodeForJavaScript(context, partId)+"');\"><img src=\"../common/images/iconActionView.gif\" border=\"0\" alt=\""+i18nViewerTip+"\" title=\""+i18nViewerTip+"\"></img></a>";
                    }
                    }
                }
                ...
                {
                    if(!bUIType)
                        viewerURL = "<a href=\"javascript:callCheckout('"+XSSUtil.encodeForJavaScript(context, objectId)  +"','view', '"+ XSSUtil.encodeForJavaScript(context, fileName)+ "', '" + XSSUtil.encodeForJavaScript(context, format) +"', null, null, null, null, '"+XSSUtil.encodeForJavaScript(context, partId)+"');\"><img src=\"../common/images/iconActionView.gif\" border=\"0\" alt=\""+i18nViewerTip+"\" title=\""+i18nViewerTip+"\"></img></a>";
                    else
                        viewerURL = "<a href=\"javascript:openViewer('"+ XSSUtil.encodeForJavaScript(context, objectId) +"','view', '"+ XSSUtil.encodeForJavaScript(context, fileName)+ "', '" + XSSUtil.encodeForJavaScript(context, format) +"', null, null, null, null, '"+XSSUtil.encodeForJavaScript(context, partId)+"');\"><img src=\"../common/images/iconActionView.gif\" border=\"0\" alt=\""+i18nViewerTip+"\" title=\""+i18nViewerTip+"\"></img></a>";
                }
                returnURL = viewerURL;
    ...
    public String showFileFormat(Context context, String[] args) throws Exception
    {
        ...
        while(mapListItr.hasNext()){
            ...
            String curFileId = (String) fieldMap.get(OBJECT_MAP_ID);
            if(fileObjectId.equals(curFileId)){
                fileFormat =  (String) fieldMap.get(CommonDocument.SELECT_FILE_FORMAT);
                break;
            }
        }
        String languageStr = (String) requestMap.get(PARAM_LIST_LANGUAGE_STR);
        languageStr = languageStr == null ? (String)((HashMap)programMap.get("requestMap")).get(PARAM_LIST_LANGUAGE_STR) : languageStr;
        if(!"".equals(fileFormat)){
    ...
    private static Map<String, Object> createAssetMap(String id ,String spaceUrl) {
        Map<String, Object> asset = new HashMap<>();
        asset.put("provider", "EV6");
        asset.put("dtype", "DOCUMENTS");
        asset.put("serverurl", spaceUrl);
        asset.put("tenant", "OnPremise");
        asset.put("contextId", "VPLMProjectLeader.Company Name.Common Space");
        asset.put("physicalid", id);
        return asset;
    }
     ...
     * @throws Exception
     */
    public static String getPdfViewUrl(Context context, String masterId) throws Exception{
        String jsonString = "";
        Properties prop = NioJDUtils.readPageObject(context, "JFJDConfig");
        String dashboardUrl = prop.getProperty("jf.dashboard.pdfUrl");
        String spaceUrl = prop.getProperty("jf.space.url");
        String tempId = MqlUtil.mqlCommand(context, false, "pri bus "+masterId+" select from.to.physicalId dump", true);
        if (UIUtil.isNotNullAndNotEmpty(tempId)){
            String[] split = tempId.split(",");
            for (String id : split) {
                String title = MqlUtil.mqlCommand(context, false, "pri bus "+id+" select attribute[Title] dump", true);
                if (title.endsWith("pdf")){
                    //获取url后面的json信息
                    // 创建输入结构的 Map 对象
                    Map<String, Object> input = new HashMap<>();
                    input.put("asset", createAssetMap(id,spaceUrl));

                    // 创建选项结构的 Map 对象
                    Map<String, Object> options = new HashMap<>();
                    options.put("loading", "autoplay");

                    // 创建最终结构的 Map 对象
                    Map<String, Object> structure = new HashMap<>();
                    structure.put("input", input);
                    structure.put("options", options);

                    // 将结构对象转换为 JSON 字符串
                    Gson gson = new Gson();
                    jsonString = gson.toJson(structure);
                    String encodedString = URLEncoder.encode(jsonString, "UTF-8");
                    jsonString = dashboardUrl+"params="+encodedString;
                    break;
                }
            }
        }
