...
var maxCount = 1;
var maxFlag = true;
var isMozilla = (typeof InstallTrigger == "object" ? true : false) && Browser.MOZILLA_FAMILY;
...
function addInputSubmitEvent(form, input) {
	  emxUICore.addEventHandler(input, "keydown", function(e) {
       e = e || window.event;
       if (e.keyCode == 13) {
       	//below line commented for IR-865710
...
function reload()
{
    ...
    if(objForm && objForm.emxTableRowId && sUrl.indexOf("emxTableRowId") == -1){
    	var tableRowId = objForm.emxTableRowId.value;		
		if( tableRowId && tableRowId.indexOf("|") > -1) {
			tableRowId =tableRowId.replace(/\|/g, encodeURIComponent("|"));
		}	
    	sUrl += "&emxTableRowId="+tableRowId;
    }
    //objForm.action = sUrl;
    ...
    for(var i=0; i < formElement.length-1; i++)
      {
       ...
       formElement.value="";
	       if("Template" === fieldName){
	    	   if(document.getElementById("calc_PreserveTaskOwner") != null && typeof document.getElementById("calc_PreserveTaskOwner") != 'undefined'){
	    		   document.querySelectorAll("#PreserveTaskOwner")[1].disabled=false;
	    	   }
	       }
   }
 }
...
function isNumeric(fieldObj){
    ...
    //Added for BUG:320716 and 367075
  	...
  	if (decSymb == "." && isComma && !isDot)
		{
           varValue = varValue.replace(",",decSymb);
		   result = !isNaN( varValue );
           varValue = varValue.replace(decSymb,",");
		}
...
function validateField(fieldObject)
{
    ...
    if(fieldObject)
    {
      ...
      //Validate for length of name field
		...
		if((fieldObject.type=="text")||(fieldObject.type=="textarea"))
		{
...
function validateForm(fromEditableTable)
{
    ...
    //20260804 update by ljr JFEditDAForm保存时统一校验DA启动时间和关闭时间
    var currentFormId = targetDoc.getElementById("formId");
    if (!fromEditableTable && currentFormId && currentFormId.value == "JFEditDAForm"
            && !validateDADateRange()) {
        return false;
    }
...
/**
 *  copy by  saveChanges chenyan
 *  复制saveChanges此函数为编辑Form保存函数 修改P1717 form 提交action
 * @param targetLocation
 */
function saveChangesECR(targetLocation)
{
        ...
        else
        {
            if (validateForm()) {
                ...
                if (validatemxLinks(formName, mode, uiType, timeStamp))
                {
					...
					// mod by chenyan 2024/08/19
                    // var url = "emxFormEditProcess.jsp"; 原参数
                    var url = "JF_ECRFormEditProcess.jsp";
        ...
        for(var n=0; n < temparray.length; n++)        
        {
           ...
           else
           {
		        if((tempobj.type=="text")||(tempobj.type=="textarea"))
		        {
					...
					formFieldValue = trimWhitespace(formFieldValue);
...
function cleanUpFormData2(timeStamp)
{
  ...
  document.location.href = 'emxFormCleanupData.jsp?timeStamp=' + timeStamp;
    ...
    if (!printDialog || printDialog.closed) {
			...
			registerChildWindows(printDialog, getTopWindow());
...
function openRenderPDFPage(timeStamp, useAdlibSetup) {
	        ...
	        if(canTableFormSubmit)
	        {
	            ...
	            if (validateForm(true))
	            {
	                ...
	                if (validatemxLinks(theForm, "edit", "table", timeStamp, objCount)) {
	                    ...
	                    if(isApplyClicked == "true" || jsDblClick()){	
    ...
    {
        if (JFProductConfigApprovalShouldInterceptCreate()) {
            if (window.JFApprovalBypass) {
                window.JFApprovalBypass = false;
            } else {
                JFProductConfigApprovalHandleCreateSubmit(isApply, targetLocation);
                return;
            }
        }
...
function checkResultCount(url,currentField,filtersArray, curFieldName){
	...
	try {
		    ...
		    var searchTimeStamp = new Date().getTime();
...
function addValueInCreateForm(id, showClear, showClearText, showAdd)
{
	...
	if(showAdd){
		...
		newId = id.substring(0, ind+1)+(maxCount);
	}else{
		if(node.type=="text"){
	...
	if(node.type=="text"){
				...
				var sizeTextField = node.getAttribute("size"); 				
				var textHTML = "<input id=\""+newId+"\"type=text name=\""+newId+"\" class=\""+className+"\" title=\""+node.title+"\""; 
				if (sizeTextField != null){				
					textHTML = textHTML + " size=\"" + sizeTextField + "\""; 
				} 
				textHTML = textHTML + "></input>";
...
function sortEmbeddedTable(timeStamp,tableName,columnName,sortDirection)
{
	if(!sortDirection){
		sortDirection="ascending";
	}
	jQuery("#"+tableName).empty();
	jQuery.ajax({
		url: "emxFormTableInclude.jsp?timeStamp="+timeStamp+"&sortColumnName="+columnName+"&sortDirection="+sortDirection+"&isEmbeddedTable=true",
		success: function(data){
			jQuery("#"+tableName).html(data);
		}
	}); 
}

//OSLC, this function delegate the call to invoke Delegated UI
function loadDelegatedUICreate(){
	//parent will be navigator in case of slide, Create form incase of popup
	if(parent.window.loadDelegatedUI){
		parent.window.loadDelegatedUI();
	}

	
}

//this function will be invoked if the Create page is loaded in a pop up
function loadDelegatedUI(){
	require([
		...
		objTest.setup();
		objTest.launchDelegatedUI(objTest.createIssuePostProcessPopup);
    });
}
//
...
function JFProductConfigApprovalInit() {
	return true;
}

function JFProductConfigApprovalShouldInterceptCreate() {
	return !!(document.forms["emxCreateForm"]
			&& JFProductConfigApprovalById("JFApprovalProductConfigId")
			&& JFProductConfigApprovalById("JFApprovalForceSubmit"));
}

function JFProductConfigApprovalHandleCreateSubmit(isApply, targetLocation) {
	window.JFApprovalLastIsApply = isApply;
	window.JFApprovalLastTargetLocation = targetLocation;
	if (typeof validateCreateForm === "function" && !validateCreateForm()) {
		if (typeof setFormSubmitAction === "function") {
			setFormSubmitAction(true);
		}
		return;
	}
	JFProductConfigApprovalPreCheck();
}

function JFProductConfigApprovalById(id) {
	return document.getElementById(id);
}

function JFProductConfigApprovalValue(id) {
	var ele = JFProductConfigApprovalById(id);
	return ele ? ele.value : "";
}

function JFProductConfigApprovalSetValue(id, value) {
	var ele = JFProductConfigApprovalById(id);
	if (ele) {
		ele.value = value || "";
	}
...
function JFProductConfigApprovalAjax(url, body, callback) {
...
function JFProductConfigApprovalShowConfirm(message, onContinue) {
	var oldMask = JFProductConfigApprovalById("JFApprovalConfirmMask");
	if (oldMask && oldMask.parentNode) {
		oldMask.parentNode.removeChild(oldMask);
	}
	var mask = document.createElement("div");
	mask.id = "JFApprovalConfirmMask";
	mask.style.cssText = "position:fixed;left:0;top:0;right:0;bottom:0;background:rgba(0,0,0,.25);z-index:9999;";
	var box = document.createElement("div");
	box.style.cssText = "width:460px;margin:130px auto 0;background:#fff;border:1px solid #b9c0c7;box-shadow:0 4px 14px rgba(0,0,0,.25);font-size:13px;";
	var msg = document.createElement("div");
	msg.style.cssText = "padding:16px;white-space:pre-line;max-height:260px;overflow:auto;";
	msg.appendChild(document.createTextNode(message || ""));
	var actions = document.createElement("div");
	actions.style.cssText = "padding:12px 16px;text-align:right;background:#f0f0f0;border-top:1px solid #d0d0d0;";
	var ok = document.createElement("button");
	ok.type = "button";
	ok.innerHTML = "继续发布";
	ok.style.cssText = "margin-right:10px;min-width:82px;height:30px;";
	var cancel = document.createElement("button");
	cancel.type = "button";
	cancel.innerHTML = "返回修改";
	cancel.style.cssText = "min-width:82px;height:30px;";
...
function JFProductConfigApprovalUploadAttachment(callback) {
	var file = JFProductConfigApprovalById("JFApprovalAttachmentFile");
	if (!file || !file.files || file.files.length === 0) {
		callback(true);
		return;
	}
	var msg = JFProductConfigApprovalById("JFApprovalAttachmentMessage");
	if (msg) {
		msg.innerHTML = "附件上传中...";
	}
	var data = new FormData();
	data.append("attachment", file.files[0]);
	JFProductConfigApprovalAjax("../common/JF_ProductConfigSubmitApprovalProcess.jsp?mode=upload", data, function (result) {
		if (result.code === "200") {
			JFProductConfigApprovalSetValue("JFApprovalAttachmentDocId", result.documentId || "");
			if (msg) {
				msg.innerHTML = "附件已上传";
			}
			callback(true);
			...
			alert(result.mess || "附件上传失败。");
			callback(false);
		}
	});
}

function JFProductConfigApprovalContinueSubmit(force) {
	JFProductConfigApprovalUploadAttachment(function (ok) {
		if (!ok) {
			return;
		}
		JFProductConfigApprovalSetValue("JFApprovalForceSubmit", force ? "true" : "false");
		window.JFApprovalBypass = true;
		if (typeof saveCreateChanges === "function") {
			saveCreateChanges(window.JFApprovalLastIsApply, window.JFApprovalLastTargetLocation);
		}
	});
}

function JFProductConfigApprovalPreCheck() {
	var productConfigId = JFProductConfigApprovalValue("JFApprovalProductConfigId");
	//20260828 update by caipan 产品配置表改为非必填，允许审批内容为空后续补充
	var body = "productConfigId=" + encodeURIComponent(productConfigId)
			+ "&servicePartsListId=" + encodeURIComponent(JFProductConfigApprovalValue("JFApprovalServicePartsListId"));
	JFProductConfigApprovalAjax("../common/JF_ProductConfigSubmitApprovalProcess.jsp?mode=preCheck", body, function (result) {
		if (result.code === "206") {
			JFProductConfigApprovalShowConfirm(result.mess || "", function () {
				JFProductConfigApprovalContinueSubmit(true);
			});
			return;
		}
		if (result.code !== "200") {
			alert(result.mess || "校验失败。");
			if (typeof setFormSubmitAction === "function") {
				setFormSubmitAction(true);
			}
			return;
		}
		JFProductConfigApprovalContinueSubmit(false);
	});
}

function jfCostCreatePre() {
	var JFConfigGroup = document.getElementById('JFConfigGroup').value
		console.log("JFConfigGroup:",JFConfigGroup);
	if(JFConfigGroup==='error'){
		//判断是否是配置类型
		alert('只可以勾选勾选一项配置新建整椅!');
		window.close();
		return false;
	}else if(JFConfigGroup==='two'){
		alert('只可以勾选勾选一项配置新建整椅!');
		window.close();
		return false;
	}
...
//add by ljr 20250820 ESO任务创建 职能与模块联动
function reloadESOFunctionModules(){
	emxFormReloadField("JF_Modules");
}

//模块与地点 部门联动
function reloadESOModulesLocations(){
	emxFormReloadField("JF_Locations");
	emxFormReloadField("JF_Department");
}
//地点与部门联动
function reloadESOLocationsDepartment(){
	emxFormReloadField("JF_Department");
}

function reloadESOR2TKOESOType() {
	const  esoType = document.getElementById('JF_ESOType');
	var value = emxFormGetValue("JF_R2_TKO").current.actual;
	if ("Yes" === value) {
		esoType.value = "TKO|Phase";
		document.getElementById('JF_ESOType').disabled = true;
	} else {
		esoType.value = "-";
		document.getElementById('JF_ESOType').disabled = false;
	}
}
...
function checkEditField() {
	// console.log("checkEditField:checkEditField");
	// console.log("checkEditField:checkEditField");
	// console.log("checkEditField:checkEditField");
	//
	// var strJFProjectPhase = emxFormGetValue("JFProjectPhase").current.actual;
	// var StrJFIsPlatformPartfieldValue = emxFormGetValue("JFIsPlatformPart").current.actual;
	// console.log("checkEditField:"+strJFProjectPhase);
	// console.log("checkEditField:"+StrJFIsPlatformPartfieldValue);
	//
	...
	var tr = document.getElementById('calc_JFIsLastQuote');
	...
	// if(selectValue == 'phase2'||selectValue == 'phase3'){
		//20260519 ljr add 加入phase2+3与之前的phase 2/3一样
	...
	var StrJFIsPlatformPartfieldValue = emxFormGetValue("JFIsPlatformPart").current.actual;
	//影响工厂
	if(StrJFIsPlatformPartfieldValue=="No"){
		var JFAffectsFactory_html = document.getElementById("JFAffectsFactory_html");
		console.log(JFAffectsFactory_html);
...
function checkEditFieldForPCR() {

	var StrJFIsPlatformPartfieldValue = emxFormGetValue("JFIsPlatformPart").current.actual;
	var StrJF_PCRIsAffectFKPIfieldValue = emxFormGetValue("JF_PCRIsAffectFKPI").current.actual;
	var StrJF_PCRIsAProductCharfieldValue = emxFormGetValue("JF_PCRIsAProductChar").current.actual;

	console.log("StrJF_PCRIsAffectFKPIfieldValue--->"+StrJF_PCRIsAffectFKPIfieldValue);
	console.log("StrJF_PCRIsAProductCharfieldValue--->"+StrJF_PCRIsAProductCharfieldValue);
	//影响工厂
	if(StrJFIsPlatformPartfieldValue=="No"||StrJFIsPlatformPartfieldValue=="N"){
		var JFAffectsFactory_html = document.getElementById("JFAffectsFactory_html");
		console.log(JFAffectsFactory_html);
		const  domAffectsFactory = document.getElementById('calc_JFAffectsFactory');
		const  JFAffectsFactory = document.getElementsByName("JFAffectsFactory")
	...
	if(StrJF_PCRIsAffectFKPIfieldValue=="Y"){
		var tr = document.getElementById("calc_JF_PCRKPI");
		tr.children[0].className = "createLabelRequired";
		validateRequiredField("JF_PCRKPI");
	}
	if(StrJF_PCRIsAProductCharfieldValue=="Y"){
		var tr = document.getElementById("calc_JF_PCRProductSAChar");
		tr.children[0].className = "createLabelRequired";
		validateRequiredField("JF_PCRProductSAChar");
	}
...
function onchangeJFProjectLevel() {
	...
	//新增刷新项目阶段   需要判断选择项目是无DV/DV 然后动态匹配Range  add by ljr 20260519
