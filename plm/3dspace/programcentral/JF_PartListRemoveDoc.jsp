...
<%!
    //日志
    private static final Logger LOGGER = LoggerFactory.getLogger("JF_PartListRemoveDoc.jsp");
%>
...
<%
    ...
    String error = EnoviaResourceBundle.getProperty(context, "emxProgramCentralStringResource", context.getLocale(), "emxProgramCentral.partList.removeDoc");
    ...
    try {
        ...
        for (String s : tableRowId) {
            ...
            if(null != mParsedRowId) {
              String  sRelId = (String)mParsedRowId.get("relId");
              if(UIUtil.isNotNullAndNotEmpty(sRelId)) {
                 String owner =  MqlUtil.mqlCommand(context, false, "print connection '" + sRelId + "' select owner dump ", true);
                 if(!currentOwner.equalsIgnoreCase(owner)){
                     strMess=error;
                 }
              }
