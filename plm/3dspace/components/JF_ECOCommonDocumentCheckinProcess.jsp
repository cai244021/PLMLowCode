...
<%
    ...
    String strRowId = (String)requestMap.get("rowId");
...
<%
    ...
    MapList mapList = parent.getRelatedObjects(context, "Reference Document", "*", boSel, null, false, true, (short) 0, "", "", 0);
...
%>
<script>
    var parentWin = getTopWindow().getWindowOpener().window;
    var parentDoc = getTopWindow().getWindowOpener().document;
    const  strMode = parentWin.editableTable.mode ;
    console.log("strMode:",strMode)
    //判判断表格是否处于编辑模式，不处于需要手动设置为编辑模式
    if (strMode != "edit"){
        parentDoc.getElementById("editButttonId").click();
    }
    parentWin.emxEditableTable.setCellValueByRowId("<%=strRowId%>","JFAttachment",`<%=buffer.toString()%>`,`<%=nameList.join(",")%>`,true);
    getTopWindow().close();
</script>
<!-- //XSSOK -->
