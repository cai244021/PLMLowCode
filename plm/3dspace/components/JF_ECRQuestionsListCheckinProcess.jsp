...
<%
        ...
        String strFileName = fileDoc == null ? "" : fileDoc.getName();
        int fileSeparatorIndex = Math.max(strFileName.lastIndexOf('/'), strFileName.lastIndexOf('\\'));
        if (fileSeparatorIndex >= 0) {
                strFileName = strFileName.substring(fileSeparatorIndex + 1);
        }
...
<script>
        ...
        console.log(parentDoc.getElementById("JFECRQQFileId2").value);
        parentDoc.getElementById("JFECRQQFileId2").value = "<%=res.get("newId")%>";
        var fileNameField = parentDoc.getElementById("JFQuestionsList");
        if (fileNameField) {
                fileNameField.value = "<%=XSSUtil.encodeForJavaScript(context, strFileName)%>";
        }
        parentDoc.getElementById("JFReplace2").innerHTML = `<%=res.get("html")%>`;
