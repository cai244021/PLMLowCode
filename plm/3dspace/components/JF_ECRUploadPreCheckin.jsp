...
<body>
<%!
    private static final Logger _logger =  LoggerFactory.getLogger("JF_ECRUploadPreCheckin");
%>
<%
    String AMP  = "&";
    String objectId = (String)emxGetParameter(request, "objectId");
    String strUploadType = (String)emxGetParameter(request, "type");
    StringBuffer sbUrl = new StringBuffer( "JF_ECRUploadCheckInFS.jsp");
    sbUrl.append("?objectId=");
    sbUrl.append(objectId);
    sbUrl.append(AMP);
    sbUrl.append("type=");
    sbUrl.append(strUploadType);
%>
          <form name="application" action="<%=XSSUtil.encodeForHTML(context, sbUrl.toString())%>" >
            <input type="hidden" name="objectId" value="<%=objectId%>" />
            <input type="hidden" name="type" value="<%=strUploadType%>" />
          </form>
          <script>
